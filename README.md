...
2. Create new environment in home directory by running command: `cd && python -m venv pseudicom_env`
3. Activate new environment by running command: `source pseudicom_env/bin/activate`
4. Install Nipype, pydicom, quickshear: `pip install nipype pydicom quickshear`
5. Download [pseuDICOM](https://github.com/can-lab/pseuDICOM/master.zip)
6. Install with
   ```
   pip install pseuDICOM-X.X.X.zip
   ```
   (replace X.X.X with latest release version)
